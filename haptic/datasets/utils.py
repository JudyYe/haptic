...
def get_aug_list(config, num, rate=1.0):
    # identity aug
    aug = 1.0, 0, False, False, 0, [1.0, 1.0, 1.0], 0., 0.
    if random.random() > rate:
        aug_list = [aug, ] * num
        return aug_list
    if config.IID_AUG == 0:  # all are correlated
        aug = do_augmentation(config)
        aug_list = [aug, ] * num
    elif config.IID_AUG == 1:  # all are independent
        aug_list = []
        for i in range(num):
            aug_list.append(do_augmentation(config))
    elif config.IID_AUG == 2:  # only correlate rotation
        aug_list = []
        for i in range(num):
            aug = do_augmentation(config)
            aug = list(aug)
        ...
        raise ValueError(f"Unknown IID_AUG {config.IID_AUG}")
    return aug_list
...
def expand_to_aspect_ratio(input_shape, target_aspect_ratio=None):
    ...
    if h_new < h or w_new < w:
        print(f'{h_new=}, {w_new=}, {h=}, {w=} bbreak')
        breakpoint()
...
def get_example(img_path: str|np.ndarray, center_x: float, center_y: float,
    ...
    # Process 3D keypoints: flip, in-plane rotation
    keypoints_3d = keypoint_3d_processing(keypoints_3d, flip_kp_permutation, rot, do_flip)
    # Process other 3D keypoints
    if other_3dpts_cam is not None:
        B, J, D = other_3dpts_cam.shape
        other_3dpts_cam = keypoint_3d_processing(other_3dpts_cam.reshape(B*J, D), flip_kp_permutation, rot, do_flip)
        other_3dpts_cam = other_3dpts_cam.reshape(B, J, D)
    ...
    rtn = [img_patch, keypoints_2d, keypoints_3d, mano_params, has_mano_params, img_size]
    if return_trans:
        rtn.append(trans)
    if other_3dpts_cam is not None:
        rtn.append(other_3dpts_cam)
    if return_orig_img:
