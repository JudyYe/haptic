...
from .utils import get_example, expand_to_aspect_ratio, get_aug_list
...
class ImageDataset(Dataset):
    def __init__(
        ...
        **kwargs,
        ...
        # filter self.data based on data['center'] > 0 and < W, H
        valid = np.ones(len(self.data["center"]), dtype=bool)
        for c, center in enumerate(self.data["center"]):
            if center[0] < 0 or center[1] < 0:
                valid[c] = False
        self.data = {k: v[valid] for k, v in self.data.items()}
        ...
        self.scale = (
            np.array(self.data["scale"]).reshape(len(self.center), -1) / 2
        )  / 100
    ...
    def __getitem__(self, idx: int) -> Dict:
        ...
        aug_bg = get_aug_list(self.cfg.DATASETS.BG_CONFIG, 1, self.cfg.DATASETS.BG_CONFIG.AUG_RATE)[0]
        ...
        # whole frame
        cfg = self.cfg
        use_dino = not cfg.MODEL.get('SHARE_BACKBONE', True)
        keep_ratio = not use_dino or cfg.DATASETS.BG_CONFIG.get("KEEP_RATIO", True)
        ...
        (
            ...
            # this does not matter if self.train is False?
            aug=aug_bg,
        )[0]        
    ...
    def load_tars_as_webdataset(
        ...
        **kwargs,
        ...
        use_dino = not cfg.MODEL.get('SHARE_BACKBONE', True)
        keep_ratio = not use_dino or cfg.DATASETS.BG_CONFIG.get("KEEP_RATIO", True)
        print(f"image loader: {keep_ratio} use dino: {use_dino}")
        ...
        dataset = dataset.map(
            lambda x: ImageDataset.process_webdataset_tar_item(
                ...
                augm_bg_config=cfg.DATASETS.BG_CONFIG,
                MEAN=MEAN,
                STD=STD,
                IMG_SIZE=IMG_SIZE,
    ...
    def process_webdataset_tar_item(
        ...
        aug = get_aug_list(augm_config, 1, )[0]
        aug_bg = get_aug_list(augm_bg_config, 1, augm_bg_config.AUG_RATE)[0]
        (
            img_patch_rgba,
            keypoints_2d,
            keypoints_3d,
