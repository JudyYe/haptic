...
class MixedWebDataset(wds.WebDataset):
    def __init__(
        self, cfg: CfgNode, dataset_cfg: CfgNode, train: bool = True, dataset_list=None
    ) -> None:
...
class HAPTICDataModule(pl.LightningDataModule):
    def __init__(self, cfg: CfgNode, dataset_cfg: CfgNode) -> None:
        """
        Initialize LightningDataModule for HAPTIC training
        Args:
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        ...
        shuffle = 250
        epoch = 5_000
        if self.cfg.webdata and self.train_dataset is None:
            # self.train_dataset = MixedWebDataset(self.cfg, self.dataset_cfg, train=True).with_epoch(epoch).shuffle(shuffle)
            self.train_dataset = MixedWebDataset(
                self.cfg, self.dataset_cfg, train=True
            ).shuffle(shuffle)
            ...
            if self.cfg.TRAIN.IMAGE_BATCH_SIZE > 0:
                # self.single_dataset = MixedWebDataset(self.cfg, self.dataset_cfg, train=True, dataset_list=self.cfg.DATASETS.IMAGE).with_epoch(epoch).shuffle(shuffle)
                self.single_dataset = (
                    MixedWebDataset(
                        self.cfg,
                        self.dataset_cfg,
                        train=True,
                        dataset_list=self.cfg.DATASETS.IMAGE,
                    )
                    .with_epoch(epoch)
                    .shuffle(shuffle)
                )
        elif not self.cfg.webdata and self.train_dataset is None:
    ...
    def train_dataloader(self) -> Dict:
        ...
        mocap_dataloader = torch.utils.data.DataLoader(
            self.mocap_dataset,
            self.cfg.TRAIN.NUM_TRAIN_SAMPLES
            * self.cfg.MODEL.NUM_FRAMES
            * self.cfg.TRAIN.BATCH_SIZE,
            shuffle=True,
            drop_last=True,
            num_workers=1,
        ...
        datasets = {
            ...
            "mocap": mocap_dataloader,
        }
        if self.cfg.TRAIN.IMAGE_BATCH_SIZE > 0:
            single_dataloader = torch.utils.data.DataLoader(
                self.single_dataset,
                self.cfg.TRAIN.IMAGE_BATCH_SIZE,
                drop_last=True,
                num_workers=4,
                prefetch_factor=self.cfg.GENERAL.PREFETCH_FACTOR,
                pin_memory=True,
            )
            datasets["single"] = single_dataloader
        return datasets
